  ...
  <div class="back">
    <div class="add-coin">
      ...
      <div class="mb-3 d-flex">
        ...
        <input type="number" class="left form-mm" @input="leftCalcExchange" v-model="inputNumber" aria-label="Text input with dropdown button">
      </div>
      <button class="btn btn-primary m-1" @click="addCoins">Добавать</button>
      <button class="close-pop" @click="$store.state.portfolio.popAddCoin = false">&times;</button>
    </div>
  </div>
</template>
    ...
    methods: {
      addCoins() {
        if(this.currentValue == 'bitcoin') {
          ...
          this.$store.state.portfolio.popAddCoin = false
